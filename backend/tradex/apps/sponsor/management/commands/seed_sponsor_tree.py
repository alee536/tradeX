...
class Command(BaseCommand):
    ...
    def _print_expected_summary(self):
        ...
        self.stdout.write(f'  Direct sponsored count     = {summary["direct_sponsored_count"]}')
        self.stdout.write(f'  Direct sales (USDT)        = ${summary["total_direct_sales_usdt"]}')
        self.stdout.write(f'  Reward percentage          = {summary["reward_percentage"]}%')
        self.stdout.write(f'  Calculated reward (USDT)   = ${summary["calculated_reward_usdt"]}')
        self.stdout.write(f'  Claimed so far (USDT)      = ${summary["claimed_so_far_usdt"]}')
        self.stdout.write(f'  Outstanding reward (USDT)  = ${summary["outstanding_reward_usdt"]}')
        self.stdout.write(f'  Threshold (USDT)           = ${summary["threshold_usdt"]}')
